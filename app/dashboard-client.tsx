...
function CostsReport({ data }: { data: DashboardData }) {
  return <section className="report-page"><div className="page-intro"><div><p className="eyebrow light">Fuel & costs</p><h2>Quick C cost analysis and fuel price decisions.</h2></div><span className="period-chip">Formula-driven margins</span></div>
    ...
    <div className="split-grid"><article className="panel"><div className="section-head compact"><div><p className="eyebrow">Daily cost report</p><h3>{data.quickC.period}</h3></div></div><div className="mini-cost-list">{data.quickC.daily.map((row) => <div key={row.date}><span>{row.date}</span><b>{moneyExact.format(row.sales)}</b><small>COGS {moneyExact.format(row.scannedCost + row.modifierCost)}</small></div>)}</div></article><article className="panel"><div className="section-head compact"><div><p className="eyebrow">Plant City fuel</p><h3>Competitor and margin tracker</h3></div></div>{data.fuelPrices.length ? <div className="fuel-cards">{data.fuelPrices.map((row) => <article key={row.grade}><div><strong>{row.grade}</strong><small>{row.date}</small></div><dl><div><dt>Cost</dt><dd>{moneyExact.format(row.cost)}</dd></div><div><dt>Nearby</dt><dd>{moneyExact.format(row.nearbyLow)}–{moneyExact.format(row.nearbyHigh)}</dd></div><div><dt>Cash / Card</dt><dd>{moneyExact.format(row.cashPrice)} / {moneyExact.format(row.cardPrice)}</dd></div><div><dt>Margin</dt><dd>{moneyExact.format(row.cashMargin)} / {moneyExact.format(row.cardMargin)}</dd></div></dl></article>)}</div> : <div className="empty-state compact-empty"><h3>No fuel-price rows yet</h3><p>Add the fuel tracker to the management Sheet to activate this section.</p></div>}</article></div>
  </section>;
}
...
function ExpensesReport({ data }: { data: DashboardData }) {
  ...
  const byType = (type: string) => data.moneyMovements.filter((row) => row.type === type).reduce((sum, row) => sum + row.amount, 0);
  return <section className="report-page"><div className="page-intro"><div><p className="eyebrow light">Purchases & expenses</p><h2>Money movements stay separate from daily sales.</h2></div><span className="period-chip">Management reporting view</span></div><div className="kpi-grid four"><article><span>Purchases</span><strong>{moneyExact.format(byType("Purchase"))}</strong><small>Supplied purchase rows</small></article><article><span>Expenses</span><strong>{moneyExact.format(byType("Expense"))}</strong><small>Supplied expense rows</small></article><article><span>House Account</span><strong>{moneyExact.format(byType("House Account"))}</strong><small>Preserved source term</small></article><article><span>Total cash out</span><strong>{moneyExact.format(totalOut)}</strong><small>Other income kept separate</small></article></div>{data.moneyMovements.length ? <div className="table-wrap"><table><thead><tr><th>Date</th><th>Store</th><th>Transaction Type</th><th>Category</th><th>Amount</th><th>Report treatment</th></tr></thead><tbody>{data.moneyMovements.map((row, index) => <tr key={`${row.type}-${row.category}-${index}`}><td>{row.date || "—"}</td><td><b>{row.store}</b></td><td>{row.type}</td><td>{row.category}</td><td>{moneyExact.format(row.amount)}</td><td>{row.type === "Other Income" ? "Income — not included in expenses" : "Cash out / management cost view"}</td></tr>)}</tbody></table></div> : <div className="empty-state"><h3>No purchase or expense rows yet</h3><p>Add approved source rows to the management Sheet to activate this report.</p></div>}<p className="source-note">This open dashboard shows the published snapshot dated {data.asOf}. The source Google Sheet remains private.</p></section>;
}
...
function MercuryReport({ data }: { data: DashboardData }) {
  ...
  return <section className="report-page"><div className="page-intro"><div><p className="eyebrow light">Mercury One</p><h2>Task monitoring for management follow-up.</h2></div><span className="period-chip">{data.mercury.total} actual tasks</span></div><div className="kpi-grid four"><article><span>Completed</span><strong>{data.mercury.completed}</strong><small>{percent.format(data.mercury.total ? data.mercury.completed / data.mercury.total : 0)} completion</small></article><article><span>In Progress</span><strong>{data.mercury.inProgress}</strong><small>Needs follow-up</small></article><article><span>Waiting for Response</span><strong>{data.mercury.waiting}</strong><small>External response pending</small></article><article className="warning-kpi"><span>Status / date conflicts</span><strong>{data.mercury.statusConflicts}</strong><small>Confirm before cycle-time reporting</small></article></div><div className="split-grid mercury-grid"><article className="panel"><div className="section-head compact"><div><p className="eyebrow">Task distribution</p><h3>Current status</h3></div></div><div className="status-bars">{statuses.map((status) => <div className="status-row" key={status.label}><div><span>{status.label}</span><strong>{status.value}</strong></div><div className="status-track"><i className={status.color} style={{ width: `${data.mercury.total ? status.value / data.mercury.total * 100 : 0}%` }} /></div></div>)}</div></article><article className="panel attention"><p className="eyebrow amber">Management exceptions</p><h3>Clean the tracker before measuring response performance.</h3><ul><li>{data.mercury.missingCompletionDate} completed tasks are missing Completion Date.</li><li>{data.mercury.statusConflicts} open tasks already contain Completion Date.</li><li>{data.mercury.missingTicketIds} tasks have no Ticket ID.</li><li>Numbered-but-empty placeholders are excluded.</li></ul></article></div><article className="panel management-note"><p className="eyebrow">Recommended weekly report</p><h3>Headline → exceptions → owner → next action → due date</h3><p className="body-copy">Task descriptions remain in the private Sheet. This website displays management-ready status totals from the published snapshot.</p></article></section>;
}

function QualityReport({ data, isPublic }: { data: DashboardData; isPublic: boolean }) {
  return <section className="report-page"><div className="page-intro"><div><p className="eyebrow light">Data quality</p><h2>Known gaps remain visible instead of being silently corrected.</h2></div><span className="status warn">{data.modelStatus}</span></div><div className="quality-grid">{data.alerts.map((alert, index) => <article className="quality-card" key={alert}><span>{String(index + 1).padStart(2, "0")}</span><div><h3>{index < 2 ? "Running-total mismatch" : index === 2 ? "Incomplete store period" : index === 3 ? "Mercury tracker conflict" : "Missing source file"}</h3><p>{alert}</p></div></article>)}</div><div className="split-grid connection-grid"><article className="panel"><p className="eyebrow">Source classification</p><h3>Manual and system reports serve different purposes.</h3><p className="body-copy">The three store PDFs are professionally formatted manual comparison reports. dayReport.pdf is the system-downloaded operational source. The fuel CSV is a separate competitor-price tracker. The two deep-research Markdown files are unrelated PhD research and were excluded.</p></article><article className="panel connection-status"><p className="eyebrow">Website access rule</p><div className="connection-badge"><i className="online" /><span>{isPublic ? "Open dashboard snapshot" : "Authenticated private delivery"}</span></div><p>{isPublic ? "No sign-in is required. The website displays the saved management snapshot; the source Google Sheet remains private." : "Report values are released after Google verifies the account and the Users sheet approves it."}</p>{!isPublic && <a className="secondary-link" href={SHEET_URL} target="_blank" rel="noreferrer">Open management Google Sheet</a>}</article></div></section>;
}

export default function DashboardClient({ data, currentUser, publicLoginUrl }: { data: DashboardData; currentUser?: AuthenticatedUser; publicLoginUrl?: string }) {
  const [tab, setTab] = useState<Tab>("overview");
  const signOut = () => window.location.assign(publicLoginUrl || "https://omuniversalllc.github.io/om-universal-operations-dashboard/");
  const isPublic = !currentUser;
  return <main className="shell"><header className="topbar"><button className="brand-button" onClick={() => setTab("overview")}><span className="brandmark">OM</span><span><small>Management reporting portal</small><strong>Universal Operations Dashboard</strong></span></button><nav aria-label="Dashboard sections">{tabs.map((item) => <button className={tab === item.id ? "active" : ""} key={item.id} onClick={() => setTab(item.id)}>{item.label}</button>)}</nav><div className="header-actions"><div className="source-state"><i className="online" /><span>{isPublic ? "Public snapshot" : "Live private Sheet"}<small>{data.asOf}</small></span></div>{currentUser && <div className="user-menu"><span>{currentUser.name || currentUser.email}<small>{currentUser.role} · {currentUser.storeAccess}</small></span><button onClick={signOut}>Sign out</button></div>}<button className="print-action" onClick={() => window.print()}>Print</button></div></header><div className="privacy-strip"><span>{isPublic ? "Open dashboard mode" : "Private authenticated mode"}</span><p>Sales, fuel, expense-category and Mercury status summaries. No inventory or audit modules.</p>{!isPublic && <a href={SHEET_URL} target="_blank" rel="noreferrer">Open Google Sheet</a>}</div>
    {tab === "overview" && <Overview data={data} setTab={setTab} />}{tab === "explorer" && <SalesExplorer data={data} />}{tab === "costs" && <CostsReport data={data} />}{tab === "expenses" && <ExpensesReport data={data} />}{tab === "mercury" && <MercuryReport data={data} />}{tab === "quality" && <QualityReport data={data} isPublic={isPublic} />}
    <footer><span>OM Universal Operations</span><p>Three-store sales + Mercury management reporting · Source terminology preserved</p><span>Version 3.1 · {isPublic ? "Open snapshot" : "Secure access"}</span></footer></main>;
}

...
export default function DashboardClient({ data, currentUser, publicLoginUrl }: { data: DashboardData; currentUser: AuthenticatedUser; publicLoginUrl?: string }) {
  ...
  return <main className="shell"><header className="topbar"><button className="brand-button" onClick={() => setTab("overview")}><span className="brandmark">OM</span><span><small>Management reporting portal</small><strong>Universal Operations Dashboard</strong></span></button><nav aria-label="Dashboard sections">{tabs.map((item) => <button className={tab === item.id ? "active" : ""} key={item.id} onClick={() => setTab(item.id)}>{item.label}</button>)}</nav><div className="header-actions"><div className="source-state"><i className="online" /><span>Live private Sheet<small>{data.asOf}</small></span></div><div className="user-menu"><span>{currentUser.name || currentUser.email}<small>{currentUser.role} · {currentUser.storeAccess}</small></span><button onClick={signOut}>Sign out</button></div><button className="print-action" onClick={() => window.print()}>Print</button></div></header><div className="privacy-strip"><span>Private authenticated mode</span><p>Sales, fuel, expense-category and Mercury status summaries. No inventory or audit modules.</p><a href={SHEET_URL} target="_blank" rel="noreferrer">Open Google Sheet</a></div>
    ...
    <footer><span>OM Universal Operations</span><p>Three-store sales + Mercury management reporting · Source terminology preserved</p><span>Version 3.0 · Secure access</span></footer></main>;
}
